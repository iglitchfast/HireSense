# SCAN.AI — Resume-to-Job-Description Alignment Assistant

Python (FastAPI) backend + vanilla HTML/CSS/JS frontend, powered by **Gemini 2.5 Flash**.
...
3. **Pass 2 (`/api/rewrite`)** — Gemini finds weak bullet points (passive voice, vague verbs,
   no metrics) and rewrites them with strong action verbs. Where there's no real metric to draw
   from, it inserts `[Insert metric: ...]` instead of inventing a number — flagged clearly in
   the UI as "FILL-IN NEEDED".
...
│   ├── gemini_client.py   # Gemini 2.5 Flash wrapper (forces structured JSON output)
│   ├── prompts.py         # Pass 1 & Pass 2 prompt templates
│   ├── schemas.py         # Pydantic models (shared request/response shapes)
│   └── routes/
│       ├── analyze.py     # POST /api/analyze  (Pass 1)
│       └── rewrite.py     # POST /api/rewrite  (Pass 2)
└── frontend/
    ├── index.html
    ├── style.css
    └── app.js
...
- [x] Structured JSON output via schema — `backend/gemini_client.py` (`response_schema`)
